...
UX_LAYER = SCREENS.UX_LAYER
UX_SCRIPT = SCREENS.UX_SCRIPT


QUALIFY = [
    (SCREENS.QUALIFY_ANCHOR, SCREENS.QUALIFY_PAYLOAD, 1),
]
...
def rebuild_screens(s, report):
    ...
    if s.count(SCREENS.SESSION_FACES_OLD) != 1:
        sys.exit("BUILD FAILED: session team-row anchor matched %d times"
                 % s.count(SCREENS.SESSION_FACES_OLD))
    s = s.replace(SCREENS.SESSION_FACES_OLD, SCREENS.SESSION_FACES_NEW)
    report.append("  screen session team rows carry both players' photos")
    return s

...
- **Player roster is a live read from August Attack's DB** (`aa_tournament_state`, see `fetchSharedPlayers()` in `americano-api.js`) — by design, on the owner's request, so the two apps' player lists always stay in sync. `GET /state` overlays the live-fetched players onto Americano's own stored state; `POST /save` strips any `players` field before persisting so a stale local copy can never get served. That shared roster is **read-only** here — edit those players in August Attack. If August Attack's app/table is ever retired or renamed, `fetchSharedPlayers()` must be repointed or Americano's player list breaks.
- **Guest players** live in `state.guests` (ids prefixed `g_`) and are Americano-only — for one-off drop-ins. They persist normally because `/save` deletes only `players`, never `guests`, so **no API change was needed** to support them. The UI merges `players + guests` into one roster for check-in and standings; the Admin tab (admin role only) adds/renames/deletes guests exclusively.
- **UI** (redesigned Sept 2026 from a Claude Design canvas): editorial layout — hairline rules instead of boxed cards, Anton display type, underline-style inputs, pill buttons, dark bottom nav with an orange active indicator. Screens: Setup/Board, Standings (+ Podium), Players (+ per-player stats & partner records), History, Admin. Scoring is a **slider** that splits the fixed match total between the two teams (plus preset split buttons), then Confirm/Update score — matches never store a total that doesn't add up. Late arrivals can join mid-session from the Board.
  - Note the design export's JS defaulted to a pink accent (`#FF3B6B`) with blue-greys while its markup used the orange/cream brand palette — the two clashed (light bottom nav on a dark page). The implementation unifies on orange/cream; don't reintroduce the pink.
- **Fairness — equal matches, and the ranking metric**: in Americano the winner is whoever accumulates the most points, so a player handed an extra match starts ~half a match total ahead before anyone hits a ball. Everyone gets the same number of matches only when `players == courts × 4` (nobody rests), or when rest divides evenly: rest slots per round are `n - 4c`, and rest shares out evenly only when `R × (n - 4c)` is a multiple of `n` (`fairRoundCounts()` computes this — e.g. 14 players on 3 courts needs 7 or 14 rounds; 20 on 3 courts needs 5, 10, 15…). Otherwise equal match counts are **arithmetically impossible**, not a scheduler bug — a per-round bench optimiser that minimises individual rest spread was tried and changed nothing in 12 configurations, because equalising rest needs the benched pairs across *all* rounds to partition the field, a global constraint no per-round choice can satisfy.
  - So `calcStandings()` picks its metric from the **schedule**, not from what's been scored (`isEvenSchedule()` / `scheduledMatchCounts()`, counted over all fixtures so the metric doesn't flip mid-session): even schedule → rank on total points; uneven → rank on **points per match**, with totals shown underneath and a note on the Standings screen. Setup warns before you start and names the round counts that would be even.
  - Measured over 20k equal-skill trials: ranking on totals gave the most-played group 63.7% of titles at 24 players / 4 courts / 6 rounds (fair share 16.7%) while the least-played group won **0.0%**; 55.8% vs 22.2% at 18/4/7. On points per match those become 10.5% and 18.7%. Residual: the per-match average slightly *over*-rewards players with fewer matches (25.0% vs 16.7% fair share at 24/4/6) because a short sample is noisier — much smaller than the bias it replaces, and in the opposite direction. Prefer a `fairRoundCounts()` round count when you want it exactly even.
- **Late arrivals and the partner guarantee**: changing *courts* keeps the same circle cycle, so no partnership ever repeats. Adding a *player* necessarily builds a different cycle, which could replay a used pairing — so `rebuildTail()` tries every starting offset in the new cycle and keeps whichever collides least with already-played pairs. Measured across 68 join scenarios (n=8..20, every join round) plus three successive joins: zero repeats. It is a best-effort search, not a proof — keep the hedged wording in the join sheet.
